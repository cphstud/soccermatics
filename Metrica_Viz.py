...
def plot_pitch_with_players( field_dimen = (106.0,68.0), field_color ='green', linewidth=2, markersize=20):
    """ plot_pitch
    ...
    ax.plot(0,0,marker="o", markersize=10, markeredgecolor="red", markerfacecolor="red")
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    ax.set_xticks([])
    ax.set_yticks([])
...
def plot_pitch( field_dimen = (110.0,72.0), field_color ='green', linewidth=2, markersize=20, fak=1):
    """ plot_pitch
    ...
    """
    fig,ax = plt.subplots(figsize=(fak*12,fak*8)) # create a figure 
    # decide what color we want the field to be. Default is green, but can also choose white
    if field_color=='green':
        ax.set_facecolor('mediumseagreen')
        lc = 'whitesmoke' # line color
