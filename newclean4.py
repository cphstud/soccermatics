...
# second try where you select certain players
def_player=list(range(2,17))
def_player.append(18)
def_player.append(19)
def_player.append(20)
striker_player=list(range(21,27))
striker_player.append(17)

cursorF2=coll.find({"type.name":"Shot", "gender":"female", "position.id" : {"$in":striker_player}})
dfF2=pd.DataFrame(list(cursorF2))    

cursorM2=coll.find({"type.name":"Shot", "gender":"male","position.id" : {"$in":def_player}, "player.name":  {"$ne":"Lionel Andrés Messi Cuccittini"}}).limit(9000)

dfM2=pd.DataFrame(list(cursorM2))
dfM2Sel=dfM2.sample(n=3750, random_state=1)
frames2=[dfF2,dfM2Sel]
dfAll2=pd.concat(frames2)
dfAllPrep2=prepDFShot(dfAll2)
dfAllPrepCopy=dfAllPrep2
dfAllPrep2=dfAllPrep2.dropna(subset=['freeze_frame'])

dfAllPrep2['shot_not_ok']=dfAllPrep2.apply(lambda row: doCalc(row), axis = 1)
wsum2=0
msum2=0
for index,row in dfAllPrep2.iterrows():
...
testDfAll['shot_not_ok']=testDfAll.apply(lambda row: doCalcDebug(row), axis = 1)
...
#
mx=dfAllPrep2.loc[dfAllPrep2['_id'] == '24e3522a-588d-46d8-ab36-28281eb98a71']
mdata=[wsum,msum,wsum2,msum2]
x=range(1,5)
col={'female':'orange','male':'blue'}
labels=['male','female']
handles = [plt.Rectangle((0,0),1,1, color=col[label]) for label in labels]
fig, ax = plt.subplots()
ax.set_ylabel('Should_have_been_passes')
ax.set_title('Scores by gender')
ax.legend(handles,labels)
plt.bar(x,mdata,color=['orange','blue','orange','blue'])
plt.show()
